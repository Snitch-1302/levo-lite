...
from openapi_schema_pydantic import OpenAPI, PathItem, Operation, Parameter, RequestBody, Response, MediaType
...
class OpenAPIGenerator:
    ...
    def generate_openapi_spec(self, title: str = "Discovered API", version: str = "1.0.0") -> OpenAPI:
        ...
        for endpoint in endpoints:
            ...
            if endpoint.method == HTTPMethod.GET:
                ...
                paths[endpoint.path].put = operation
            elif endpoint.method == HTTPMethod.DELETE:
                paths[endpoint.path].delete = operation
            elif endpoint.method == HTTPMethod.PATCH:
                paths[endpoint.path].patch = operation
            elif endpoint.method == HTTPMethod.HEAD:
                paths[endpoint.path].head = operation
            elif endpoint.method == HTTPMethod.OPTIONS:
                paths[endpoint.path].options = operation
        ...
        openapi_spec = OpenAPI(
            ...
            security=self._create_security_schemes(endpoints)
        )
    ...
    def _create_security_schemes(self, endpoints: List[APIEndpoint]) -> Optional[List[Dict[str, List[str]]]]:
    ...
    def _generate_schema_name(self, path: str, suffix: str) -> str:
        ...
        path_parts = path.strip('/').split('/')
        schema_name = ''.join(part.capitalize() for part in path_parts)
